...
Base* FactoryCreater(const initializer_list<Param>& plist) // Bad: using raw pointer. unique_ptr must be used instead 
{
	return new Child(plist); 
}	
...
{
	cout << sInfo << endl;
	vector< Base* > v; // Bad: using raw pointer. unique_ptr must be used instead
	...
	return 0;
} //Ops! we forgot to free mamory. Memory leaks.
