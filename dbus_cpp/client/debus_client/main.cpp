...
int main(int argc, char *argv[]) {
  ...
  concatenatorProxy->uponSignal("concatenated")
      .onInterface(interfaceName)
      .call([](const std::string &str) { onConcatenated(str); });
  ...
  // Invoke concatenate on given interface of the object
  {
    ...
    std::cout << "Got: " << concatenatedString << std::endl;
  }

  // Invoke concatenate again, this time with no numbers and we shall get an
  // error
  {
    try {
      concatenatorProxy->callMethod("concatenate")
          .onInterface(interfaceName)
          .withArguments(std::vector<int>(), separator);
      assert(false);
